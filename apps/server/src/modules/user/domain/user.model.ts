import {
  Column,
  CreateDateColumn,
  DeleteDateColumn,
  Entity,
  JoinColumn,
  ManyToOne,
  OneToMany,
  PrimaryGeneratedColumn,
  UpdateDateColumn,
} from 'typeorm'

import { Notification } from '../../../modules/notification/domain'

import { Account } from '../../../modules/account/domain'

import { Goal } from '../../../modules/goal/domain'

import { Allowance } from '../../../modules/allowance/domain'

import { Contentaccess } from '../../../modules/contentaccess/domain'

export enum UserStatus {
  VERIFIED = 'VERIFIED',
  CREATED = 'CREATED',
}

@Entity()
export class User {
  @PrimaryGeneratedColumn('uuid')
  id: string

  @Column({ unique: true })
  email: string

  @Column()
  name: string

  @Column({ nullable: true })
  pictureUrl?: string

  @Column({ select: false, nullable: true })
  password: string

  @Column({ enum: UserStatus, default: UserStatus.VERIFIED })
  status: UserStatus

  @OneToMany(() => Account, child => child.user)
  accounts?: Account[]

  @OneToMany(() => Goal, child => child.user)
  goals?: Goal[]

  @OneToMany(() => Allowance, child => child.child)
  allowancesAsChild?: Allowance[]

  @OneToMany(() => Contentaccess, child => child.user)
  contentaccesss?: Contentaccess[]

  @OneToMany(() => Notification, notification => notification.user)
  notifications?: Notification[]

  @CreateDateColumn()
  dateCreated: string

  @UpdateDateColumn()
  dateUpdated: string

  @DeleteDateColumn()
  dateDeleted: string
}
